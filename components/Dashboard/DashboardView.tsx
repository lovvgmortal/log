...
import { Project, Folder, ScriptDNA } from '../../types';
...
export const DashboardView: React.FC<DashboardViewProps> = ({ projects, folders, dnas, onOpenProject, onNewProject, onDeleteProject, onDeleteFolder, onRenameProject, onCreateFolder, onMoveProject }) => {
    ...
    const [confirmDialog, setConfirmDialog] = useState<{ isOpen: boolean; itemId: string; itemName: string; type: 'project' | 'folder' }>({
        isOpen: false,
        itemId: '',
        itemName: '',
        type: 'project'
    ...
    return (
        <div className="animate-in fade-in duration-500">
            ...
            {!activeFolderId && !searchQuery && (
                <DashboardStats projects={projects} dnas={dnas} />
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                {!activeFolderId && displayedFolders.map(folder => (
                    <div key={folder.id} onClick={() => { setActiveFolderId(folder.id); setSearchQuery(""); }} className="group bg-zinc-900/30 border border-white/5 rounded-lg p-3 hover:bg-zinc-900/60 hover:border-white/20 transition-all cursor-pointer relative overflow-hidden flex flex-col justify-center min-h-[100px]">
                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 z-10" onClick={(e) => e.stopPropagation()}>
                            <button onClick={() => { setEditingId(folder.id); setTempName(folder.name); }} className="p-1 hover:bg-white/10 rounded text-zinc-500 hover:text-white" title="Rename"><EditIcon className="w-3 h-3" /></button>
                            <button onClick={() => setConfirmDialog({ isOpen: true, itemId: folder.id, itemName: folder.name, type: 'folder' })} className="p-1 hover:bg-white/10 rounded text-zinc-500 hover:text-red-400" title="Delete"><TrashIcon className="w-3 h-3" /></button>
                        </div>
                        <div className="w-8 h-8 bg-blue-500/10 rounded-md flex items-center justify-center mb-2 group-hover:bg-blue-500/20 transition-colors"><FolderIcon className="w-4 h-4 text-blue-400" /></div>
                        {editingId === folder.id ? (
                            <input autoFocus value={tempName} onChange={e => setTempName(e.target.value)} onBlur={() => { onRenameProject(folder.id, tempName); setEditingId(null); }} onKeyDown={e => e.key === 'Enter' && (onRenameProject(folder.id, tempName), setEditingId(null))} onClick={e => e.stopPropagation()} className="bg-black/50 border border-zinc-600 rounded px-1.5 py-1 text-white font-bold w-full mb-0.5 text-xs" />
                        ) : (
                            <h3 className="text-sm font-bold text-white mb-0.5 group-hover:text-blue-300 transition-colors">{folder.name}</h3>
                        )}
                        <p className="text-[9px] text-zinc-500">{projects.filter(p => p.folderId === folder.id).length} projects</p>
                    </div>
                ))}

                {displayedProjects.map(project => (
                    <div key={project.id} onClick={() => onOpenProject(project.id)} className="group bg-zinc-900/50 border border-white/5 rounded-lg p-3 hover:bg-zinc-900 hover:border-white/10 transition-all cursor-pointer relative overflow-hidden">
                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 z-10">
                            <button onClick={(e) => handleStartRename(e, project)} className="p-1 hover:bg-white/10 rounded text-zinc-500 hover:text-white" title="Rename"><EditIcon className="w-3 h-3" /></button>
                            <div className="relative">
                                <button onClick={(e) => { e.stopPropagation(); setMovingProjectId(movingProjectId === project.id ? null : project.id); }} className={`p-1 hover:bg-white/10 rounded transition-colors ${project.folderId ? 'text-blue-400' : 'text-zinc-500 hover:text-blue-400'}`} title="Move to folder"><FolderIcon className="w-3 h-3" /></button>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); setConfirmDialog({ isOpen: true, itemId: project.id, itemName: project.name, type: 'project' }); }} className="p-1 hover:bg-white/10 rounded text-zinc-500 hover:text-red-400" title="Delete"><TrashIcon className="w-3 h-3" /></button>
                        </div>
                        <div className="flex flex-col h-full justify-between">
                            <div>
                                <div className="mb-2"><span className={`text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border ${project.data.result ? 'bg-green-500/10 text-green-400 border-green-500/20' : project.data.blueprint ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}>{project.data.result ? 'Completed' : project.data.blueprint ? 'In Progress' : 'Draft'}</span></div>
                                {editingId === project.id ? (
                                    <input autoFocus value={tempName} onChange={e => setTempName(e.target.value)} onBlur={() => handleFinishRename(project.id)} onKeyDown={e => e.key === 'Enter' && handleFinishRename(project.id)} onClick={e => e.stopPropagation()} className="bg-black border border-zinc-600 rounded px-1.5 py-1 text-white font-bold w-full mb-1.5 text-xs" />
                                ) : (
                                    <h3 className="text-sm font-bold text-white mb-1.5 line-clamp-1">{project.name || "Untitled Project"}</h3>
                                )}
                                <p className="text-[10px] text-zinc-500 line-clamp-2">{project.data.userDraft.description || project.data.userDraft.title || "No description provided."}</p>
                            </div>
                            <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
                                <span className="text-[9px] text-zinc-600">{new Date(project.updatedAt).toLocaleDateString()}</span>
            ...
            <ConfirmDialog
                ...
                onConfirm={() => {
                    if (confirmDialog.type === 'folder') {
                        ...
                        onDeleteProject(fakeEvent, confirmDialog.itemId);
                    }
                    setConfirmDialog({ isOpen: false, itemId: '', itemName: '', type: 'project' });
                }}
                onCancel={() => setConfirmDialog({ isOpen: false, itemId: '', itemName: '', type: 'project' })}
            />
        </div>
    );
};
