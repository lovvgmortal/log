...
import { ScriptDNA, ScoringTemplate, ScoringResult, OutputLanguage, ScoringCriterion, ScriptSection } from '../../types';
...
interface Props {
  ...
  apiKey: string;
  // NEW: For regenerate functionality
...
export const ScoringPanel: React.FC<Props> = ({
  ...
  scriptSections, onRegenerateSection
}) => {
  ...
  const handleApplySuggestion = async (sectionId: string, item: any) => {
    if (!onRegenerateSection) return;
    setRegeneratingId(sectionId);

    const prompt = `Improve this section to address the following:

**Current Issue**: ${item.reasoning}

**Required Improvement**: ${item.improvement_tip || item.reasoning}

Rewrite the section maintaining the overall message but implementing the suggested improvements. Keep the tone and style consistent with the rest of the script.`;

    try {
      await onRegenerateSection(sectionId, prompt);
    } catch (e: any) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
      ...
      {lastScore && (<div className="border-t border-zinc-800 pt-6 animate-in slide-in-from-bottom-2"><div className="flex items-center justify-between mb-6"><div><h4 className="text-zinc-400 text-xs font-bold uppercase tracking-widest mb-1">Total Score</h4><div className={`text-4xl font-black ${getColor(lastScore.total_score || 0).split(' ')[0]}`}>{typeof lastScore.total_score === 'number' ? lastScore.total_score : 0}/100</div></div><div className="text-right"><span className="text-[10px] text-zinc-600 block mb-1">Analyzed at</span><span className="text-xs text-zinc-400 font-mono">{new Date(lastScore.timestamp).toLocaleTimeString()}</span></div></div><div className="space-y-4 mb-6">{(lastScore.breakdown || []).map((item: any, i: number) => { const section = scriptSections?.[i]; return (<div key={i} className="bg-black/30 rounded-xl p-4 border border-zinc-800"><div className="flex justify-between items-center mb-2"><span className="font-bold text-sm text-white">{item.criteria}</span><span className={`px-2 py-0.5 rounded text-xs font-bold border ${getColor(item.score)}`}>{item.score}</span></div><p className="text-xs text-zinc-400 mb-2">{item.reasoning}</p><div className="flex gap-2 items-start bg-blue-500/5 p-2 rounded border border-blue-500/10"><SparklesIcon className="w-3 h-3 text-blue-400 mt-0.5 flex-shrink-0" /><p className="text-[10px] text-blue-300 font-medium">Tip: {item.improvement_tip || item.reasoning}</p></div>{item.score < 80 && section && onRegenerateSection && (<button onClick={() => handleApplySuggestion(section.id, item)} disabled={regeneratingId === section.id} className="mt-3 w-full py-2.5 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg text-white text-sm font-semibold hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-purple-500/20">{regeneratingId === section.id ? (<><SpinnerIcon className="w-4 h-4 animate-spin" />Regenerating...</>) : (<><RefreshIcon className="w-4 h-4" />Apply Suggestion & Regenerate</>)}</button>)}</div>); })}</div><div className="bg-zinc-950 rounded-xl p-4 border border-zinc-800"><h5 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">Overall Verdict</h5><p className="text-sm text-zinc-300 leading-relaxed italic">"{lastScore.overall_feedback || (lastScore as any).verdict}"</p></div></div>)}
    </div>
  );
};
