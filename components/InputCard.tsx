  ...
  return (
    <div className={`bg-zinc-900 border border-zinc-800 rounded-xl p-6 transition-all duration-300 hover:border-zinc-600 hover:shadow-lg group relative ${className}`}>
      ...
      {scriptNumber !== undefined && (
        <div className="absolute -top-3 -left-3 bg-blue-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg border-2 border-zinc-900 z-20">
          #{scriptNumber}
        </div>
      )}
          ...
          {showUrl && (
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  className="w-full bg-black/20 border-b border-zinc-700 pl-10 pr-20 py-2 text-blue-400 placeholder-zinc-700 focus:outline-none focus:border-blue-500 focus:bg-black/40 transition-all text-xs font-mono rounded-t-md h-[34px]"
                  placeholder={placeholders?.url || "https://youtube.com/watch?v=..."}
                  value={data.url || ''}
                  onChange={(e) => onChange(data.id, 'url', e.target.value)}
                />
                <GlobeIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                {onFetchComments && (
                  <button
                    onClick={handleFetchClick}
                    disabled={isFetching || !data.url}
                    className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 rounded text-blue-400 text-[10px] font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5"
                    title="Fetch Top Comments"
                  >
                    {isFetching ? <SpinnerIcon className="w-3 h-3 animate-spin" /> : '↓'}
                  </button>
                )}
              </div>
            </div>
