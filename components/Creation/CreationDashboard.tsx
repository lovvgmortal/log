...
import { DNASelector } from '../DNASelector';
...
export const CreationDashboard: React.FC<CreationDashboardProps> = ({
  ...
  const updateData = (updates: Partial<typeof data>) => {
    const updatedProject = { ...project, data: { ...data, ...updates }, updatedAt: Date.now() };
  ...
  const handleUpdateResultSection = (id: string, newContent: string) => {
    ...
    // Clear scoringResult when content changes - old score is invalid for new content
    const updatedSections = data.result.rewritten.script_sections.map(s =>
      s.id === id ? { ...s, content: newContent, scoringResult: undefined } : s
    );
  ...
  // NEW: Handle regenerate from scoring suggestions
  const handleRegenerateWithSuggestion = async (sectionId: string, suggestion: string) => {
    // Clear score (content is changing)
    if (data.result?.fullScriptScore) {
      updateData({
        result: {
          ...data.result,
          fullScriptScore: undefined
        }
      });
    }

    // Call existing refine handler
    await handleRefineResultSection(sectionId, suggestion);
  };

  // NEW: Handle scoring for the FULL SCRIPT
  // Logic verified for syntax correctness
  const handleScoreFullScript = async () => {
    ...
    try {
      ...
      // Deep clone result to avoid reference issues
      const clonedResult = JSON.parse(JSON.stringify(result));

      // Update result with new score
      const updatedResult = {
        ...data.result,
        fullScriptScore: clonedResult
      };

      // CRITICAL FIX: Also update the CURRENT version in resultVersions
      let updatedVersions = data.resultVersions || [];
      if (updatedVersions.length > 0) {
        const currentVersionIndex = updatedVersions.length - 1;
        updatedVersions = updatedVersions.map((v, idx) =>
          idx === currentVersionIndex
            ? { ...v, data: { ...v.data, fullScriptScore: clonedResult } }
            : v
        );
      }
      ...
      updateData({
        ...
        resultVersions: updatedVersions
      });
  ...
  const handleUpdateScore = (result: ScoringResult) => {
    ...
    const updatedResult = {
      ...data.result,
      fullScriptScore: result
    };

    // CRITICAL: Also update the CURRENT version in resultVersions
    let updatedVersions = data.resultVersions || [];
    if (updatedVersions.length > 0) {
      // Find the version that matches current result (usually the last one)
      const currentVersionIndex = updatedVersions.length - 1;
      updatedVersions = updatedVersions.map((v, idx) =>
        idx === currentVersionIndex
          ? { ...v, data: { ...v.data, fullScriptScore: result } }
          : v
      );
    }
    ...
    <div className="animate-in fade-in">
      <div className="sticky top-0 z-50 bg-black/80 backdrop-blur-xl border-b border-white/5 mb-8 -mx-8 px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 relative z-[60]">
          <div className="flex items-center gap-2 px-3 py-2 bg-zinc-900 border border-zinc-800 rounded-lg"><span className="text-[10px] font-bold text-zinc-500 uppercase">Writer AI:</span><select value={data.selectedModel || GEMINI_MODEL} onChange={(e) => updateData({ selectedModel: e.target.value })} className="bg-zinc-900 text-white text-xs font-bold outline-none cursor-pointer border-none p-0 focus:ring-0 appearance-none min-w-[120px] [&>option]:bg-zinc-900 [&>option]:text-white [&>option]:py-2">{AI_MODELS.map(m => (<option key={m.id} value={m.id}>{m.name}</option>))}</select></div>
      ...
      <div className="w-full mx-auto pb-24 px-4">
        {data.step === 'input' && (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 animate-in slide-in-from-left-4 duration-500">
            ...
            <div className="xl:col-span-7 space-y-8">
              {/* Use DNASelector Component */}
              <DNASelector
                dnas={globalDNAs || []}
                selectedDNA={data.selectedDNA}
                onSelect={(dna) => handleInputChange({ selectedDNA: dna })}
              />

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-6 border-t border-white/5">
                <section>
                  <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4">
                    <span className="w-2 h-2 bg-green-500 rounded-full"></span> Viral References
                  </h3>
                  <div className="space-y-4">{(data.virals || []).map(v => (
                    <InputCard key={v.id} data={v} onChange={(id, f, val) => handleUpdateContentPiece('virals', id, f, val)} onRemove={(id) => handleRemoveContentPiece('virals', id)} onFetchComments={(id, url) => handleFetchComments('virals', id, url)} isRemovable showUrl showComments compact />
                  ))}</div>
                  <button onClick={() => handleAddContentPiece('virals')} className="mt-4 w-full text-xs bg-zinc-800 px-3 py-2.5 rounded-lg text-white hover:bg-zinc-700 flex items-center justify-center gap-1.5 border border-zinc-700 hover:border-zinc-600 transition-all">
                    <PlusIcon className="w-3 h-3" /> Add Viral
                  </button>
                </section>

                <section>
                  <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4">
                    <span className="w-2 h-2 bg-red-500 rounded-full"></span> Flop References (Avoid)
                  </h3>
                  <div className="space-y-4">{(data.flops || []).map(v => (
                    <InputCard key={v.id} data={v} onChange={(id, f, val) => handleUpdateContentPiece('flops', id, f, val)} onRemove={(id) => handleRemoveContentPiece('flops', id)} onFetchComments={(id, url) => handleFetchComments('flops', id, url)} isRemovable showUrl showComments compact />
                  ))}{(!data.flops || data.flops.length === 0) && (<p className="text-xs text-zinc-600 italic">No flop references added. (Optional)</p>)}</div>
                  <button onClick={() => handleAddContentPiece('flops')} className="mt-4 w-full text-xs bg-zinc-800 px-3 py-2.5 rounded-lg text-white hover:bg-zinc-700 flex items-center justify-center gap-1.5 border border-zinc-700 hover:border-zinc-600 transition-all">
                    <PlusIcon className="w-3 h-3" /> Add Flop
                  </button>
                </section>
              ...
              <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 shadow-2xl sticky top-24">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-black border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500 min-h-[80px]"
                  />
                </div>
        ...
        {data.step === 'result' && data.result && (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 animate-in slide-in-from-right-4 duration-500">
            ...
            <div className="xl:col-span-3"><div className="sticky top-24">{(() => {
              return <ScoringPanel
                fullScript={(data.result.rewritten.script_sections || []).map(s => s.content).join('\n\n')}
                dna={data.selectedDNA}
                savedTemplates={globalScoringTemplates || []}
                lastScore={data.result.fullScriptScore}
                language={data.language}
                onSaveTemplate={handleSaveScoringTemplate}
                onUpdateScore={handleUpdateScore}
                onDeleteTemplate={handleDeleteScoringTemplate}
                apiKey={userSettings?.openrouter_key || ''}
                scriptSections={data.result.rewritten.script_sections}
                onRegenerateSection={handleRegenerateWithSuggestion}
              />;
            })()}</div></div>
          </div>
        )}
