...
export const NotesView: React.FC<NotesViewProps> = ({ notes, onCreate, onUpdate, onDelete }) => {
    ...
    const [confirmDialog, setConfirmDialog] = useState<{ isOpen: boolean; noteId: string; noteTitle: string }>({
        isOpen: false,
        noteId: '',
        noteTitle: ''
    });
    ...
    return (
        <div className="animate-in fade-in duration-500">
                    ...
                    return (
                        <div key={note.id} className={`group ${colorClass} border rounded-xl p-5 transition-all relative flex flex-col h-[280px] hover:shadow-lg hover:-translate-y-1`}>
                            <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-all z-20">
                                ...
                                <button onClick={() => setConfirmDialog({ isOpen: true, noteId: note.id, noteTitle: note.title || 'Untitled Note' })} className="p-1.5 rounded bg-black/20 hover:bg-red-500 text-current hover:text-white transition-colors"><TrashIcon className="w-3.5 h-3.5" /></button>
                            </div>
            ...
            <ConfirmDialog
                ...
                danger={true}
                onConfirm={() => {
                    onDelete(confirmDialog.noteId);
                    setConfirmDialog({ isOpen: false, noteId: '', noteTitle: '' });
                }}
                onCancel={() => setConfirmDialog({ isOpen: false, noteId: '', noteTitle: '' })}
            />
        </div>
    );
};
