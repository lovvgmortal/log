...
import { WRITER_SYSTEM_PROMPT, constructSectionPrompt, REFINEMENT_PROMPT, getLanguageInstruction, constructFlowNodePrompt, FLOW_NODE_SYSTEM_PROMPT } from "./prompts/index";
...
export const refineSection = async (section: any, instruction: string, context: string, lang: OutputLanguage, apiKey: string, modelId?: string) => {
  const targetModel = modelId || GEMINI_MODEL;
  const systemPrompt = WRITER_SYSTEM_PROMPT;
  const userPrompt = REFINEMENT_PROMPT(
    instruction,
    section.content,
    context, // preContext
    "",      // postContext (empty if not available)
    `Language: ${lang}. Follow the original section's word count.`
  );
  return await generateContentViaOpenRouter(targetModel, systemPrompt, userPrompt, apiKey);
};
